...
obj/machinery/computer/stage/proc/toggle_automate_acts(user, var/change_override = TRUE)
	...
	if(!announcement_made)
		priority_announce("The 2019 Winter Ball Talent Show is starting! Please make your way down to the 1st floor to the stage.", null, 'sound/ai/default/commandreport.ogg')
	...
	for(var/ckey in all_stage_acts)
		...
		for(var/mob/M in all_actors())
			...
			RegisterSignal(M, COMSIG_LIVING_DEATH, .proc/on_death) // Need to listen to see if the person died.
...
/obj/machinery/computer/stage/proc/Reset_Stage(user, ckey, var/datum/outfit/stage/current_act, var/completed = TRUE)
	...
	if(!waiting_for_actor)
		priority_announce("Thank you to [capitalize(current_act.ckey[1])] for their performance!", title ="", sound ='sound/misc/clap1.ogg')
